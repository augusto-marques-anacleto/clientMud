...
class janelaMud(wx.Frame):
	...
	def enviaTexto(self, evento):
		...
		def pronto():
			return cliente.ativo and not cliente.eof
		if cod == wx.WXK_RETURN and mod == wx.MOD_CONTROL:
			self.entrada.SetValue(self.entrada.GetValue()+"\n")
			self.entrada.SetInsertionPointEnd()
			return
		if cod == wx.WXK_RETURN and mod == wx.MOD_SHIFT:
			if not pronto():
			...
			if self.entrada.GetValue() != "":
				...
				if hasattr(self, "texto") and self.texto:
					cliente.enviaComando(self.texto)
			return
		if cod == wx.WXK_RETURN and mod == wx.MOD_NONE:
			if not pronto():
				self.perguntaReconexao()
				return
			cliente.enviaComando(self.entrada.GetValue())
	...
	def proximoComando(self):
		if self.indexComandos <len(self.comandos):
			self.indexComandos+=1
		if self.indexComandos <= len(self.comandos) -1:
			self.entrada.SetValue(self.comandos[self.indexComandos])
		elif self.indexComandos >len(self.comandos)-1:
			self.entrada.Clear()
	...
	def carregaTimers(self):
		timer_configs = self.json_personagem.get('timers', [])
